...
public class UniBlocksUNetCom : NetworkBehaviour, IUniblockUnetServer 
{
    ...
    protected UniblocksUNetClient myClient;

    public static UniBlocksUNetCom Instance;
    void Awake()
    {
        Instance = this;
    ...
    public override void OnStartClient()
    {
        ...
        if (!isServer)
        {
            Engine.SaveVoxelData = false; // disable local saving for pure client(that is not with server)
        }
    }
    #endregion
